package imau.visualization.images;
...
public class PNGImageData implements LoadableImageData {
    ...
    private void readIHDR() throws IOException {
        ...
        if (buffer[8] != 8) {
            throw new IOException("Unsupported bit depth: " + buffer[8] + ". Only 8-Bit is supported.");
        }
    ...
    public ByteBuffer loadImage(InputStream fis, boolean flipped, boolean forceAlpha, int[] transparent)
        ...
        scratch = Buffers.newDirectByteBuffer(texWidth * texHeight * perPixel);
        ...
        if (!hasAlpha() && forceAlpha) {
            ByteBuffer temp = Buffers.newDirectByteBuffer(texWidth * texHeight * 4);
            for (int x = 0; x < texWidth; x++) {
                for (int y = 0; y < texHeight; y++) {
